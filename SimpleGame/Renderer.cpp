...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_FlagTexture = CreatePngTexture("./flag.png", GL_NEAREST);
	...
	m_ParticleTexture = CreatePngTexture("./Particle1-1.png", GL_NEAREST);
	...
	int index = 0;
	for (int i = 0; i < 100; i++) {
		float x = 2 * ((float)rand() / (float)RAND_MAX) - 1.f;
...
void Renderer::DrawParticle()
{
	...
	int a_TexLoc = glGetAttribLocation(shader, "a_Tex");
	...
	glEnableVertexAttribArray(a_TexLoc);
	...
	glVertexAttribPointer(a_TexLoc, 2, GL_FLOAT, GL_FALSE, sizeof(float) * stridesize, (GLvoid*)(sizeof(float) * 15));
	...
	glDisableVertexAttribArray(a_TexLoc);
...
void Renderer::DrawTexture(float x, float y, float sx, float sy, GLuint TexID, GLuint TexID1, GLuint method)
{
	...
	int attribPos = glGetAttribLocation(shader, "a_Pos");
...
void Renderer::DrawBloomParticle()
{
	...
	DrawTexture(0, 0, 1, 1, m_HDRRT0_1, 0, 0);
...
{
	int floatCountPerVertex = 3 + 1 + 4 + 2 + 3 + 1 + 1 + 2; 
	//x, y, z, value, r, g, b, a, (sTime, lifeTime), vx, vy, vz, mass, period, tx, ty
